...
static BPatch_process * get_dyninst_process_handle(struct ust_app *app,
		BPatch *bpatch)
{
	DBG("Retrieving process handle if already created");
	if(app->dyninst_process_handle == NULL)
	{
		DBG("process handle not found. Creating a new one");
		app->dyninst_process_handle =
		(void *) bpatch->processAttach(NULL, app->pid);
	}

	return (BPatch_process*)app->dyninst_process_handle;
}
void destroy_dyninst_process_handle(struct ust_app *app)
{
	if(app->dyninst_process_handle != NULL)
	{
		DBG("Detaching from process %d", app->pid);
		BPatch_process *process = (BPatch_process*)app->dyninst_process_handle;
		process->detach(true);
	}
	app->dyninst_process_handle = NULL;
}
void allo(BPatch_process *proc, BPatch_thread *thre)
{}
BPatch bpatch;
pthread_t thr;
		...
		uint64_t offset)
{
	...
	process = get_dyninst_process_handle(app, &bpatch);
